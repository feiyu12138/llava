...
deepspeed llava/train/train_mem.py \
    ...
    --bf16 True \
    --output_dir  $ROOT_WEIGHT/llava-v1.5-7b-finetune-stride-$stride-layer-$layer-grouping-$grouping-unified_vpe-$unified_vpe-progressive-$pivot \
    --num_train_epochs 1 \
    ...
    --lazy_preprocess True \
    --report_to wandb \
    --run_name pool64layer01d_progressive_4600 \
    --stride $stride \
    --layer $layer \
    --grouping $grouping \
    --unified_vpe $unified_vpe \
    ...
    --pivot $pivot \
    1> /data/datasets/jchen293/logs/exp/llava/$grouping-stride-$stride-layer-$layer-progressive-$pivot.out \
    2> /data/datasets/jchen293/logs/exp/llava/$grouping-stride-$stride-layer-$layer-progressive-$pivot.err
