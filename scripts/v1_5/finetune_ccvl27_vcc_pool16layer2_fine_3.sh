...
grouping=attn
num_fine_blocks=3
name=llava-v1.5-7b-pretrain-stride-$stride-layer-$layer-grouping-$grouping
deepspeed llava/train/train_mem.py \
    ...
    --bf16 True \
    --output_dir /data/luoxin/weights/llava/checkpoint/llava-v1.5-7b-pretrain-stride-$stride-layer-$layer-grouping-$grouping \
    --num_train_epochs 1 \
...
deepspeed llava/train/train_mem.py \
    ...
    --vision_tower openai/clip-vit-large-patch14-336 \
    --pretrain_mm_mlp_adapter /data/luoxin/weights/llava/checkpoint/llava-v1.5-7b-pretrain-stride-$stride-layer-$layer-grouping-$grouping/mm_projector.bin \
    --mm_projector_type mlp2x_gelu \
    --mm_vision_select_layer -2 \
    --mm_use_im_start_end False \
    --mm_use_im_patch_token False \
    ...
    --layer $layer \
    --grouping $grouping \
    > /data/luoxin/output/llava/log/$name.log
