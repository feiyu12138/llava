    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # shape: next_token_scores:[3,32000]
            if not self.cot_decoding:
                ...
                if decoding_step == 1:
                    ...
                    next_token_scores_, next_tokens_ = torch.topk(
                        -1*next_token_scores, 1, dim=1, largest=True, sorted=True
                    )
                    next_token_scores_ = -1 * next_token_scores_
                    next_token_scores, next_tokens = torch.topk(
                        next_token_scores, 1, dim=1, largest=True, sorted=True
                    )
                    
                    next_token_scores = torch.cat([next_token_scores, next_token_scores_], dim=1)
                    next_tokens = torch.cat([next_tokens, next_tokens_], dim=1)
                    next_token_scores = next_token_scores.view(1,-1)
                    next_tokens = next_tokens.view(1,-1)
                
                    next_indices = torch.arange(next_tokens.shape[1]//2).repeat(2).view(1,-1)
            
            #shape: next_tokens:[1,6]; input_ids:[3,1], next_token_scores:[1,6]; next_indices:[1,6]
            # stateless
