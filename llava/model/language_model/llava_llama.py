    ...
    def forward(
        ...
        if self.viz and self.images_idx is not None:
            ...
            for idx, map in enumerate(self.attention_maps):
                ...
                attn_curve_visual.append(map[-1,35:35+576].mean())
            ...
            # plt.close()
            plt.figure()
            plt.plot(attn_curve_visual,marker='o',label='Visual Tokens',linestyle='--')
            plt.title('Attention to Visual Tokens v.s. System Prompt Tokens on each layer')
            plt.xlabel('Transformer Layer id')
            plt.ylabel('Attention')
            plt.grid(True, which='both', linestyle='--', linewidth=0.5)
            plt.plot(attn_curve_system,marker='x',label='System Prompt Tokens')
            plt.xlabel('Transformer Layer id')
            plt.legend() 
            plt.yscale('log')
            plt.ylabel('Average Attention')
            plt.savefig(f'{self.savedir}/attention_curve_log.png',dpi=300)
            plt.close()
            # plt.figure()
