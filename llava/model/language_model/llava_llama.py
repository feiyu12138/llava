...
class AdaptiveLlamaSdpaAttention(LlamaSdpaAttention):
    ...
    def forward(
        ...
        if self.viz:
            ...
            # self.std.update(
            #     calc_qkvs_std(query_states[:,:,self.images_idx:self.images_idx+self.visual_length], 
            #                   key_states[:,:,self.images_idx:self.images_idx+self.visual_length], 
            #                   value_states[:,:,self.images_idx:self.images_idx+self.visual_length], 
            #                   hidden_states[:,self.images_idx:self.images_idx+self.visual_length])
            # )
            ...
            #     calc_qkvs_std(query_states[:,:,self.images_idx+self.visual_length:],
            #                   key_states[:,:,self.images_idx+self.visual_length:],
            #                   value_states[:,:,self.images_idx+self.visual_length:],
            #                     hidden_states[:,self.images_idx+self.visual_length:])
            # )
        # SDPA with memory-efficient backend is currently (torch==2.1.2) bugged with non-contiguous inputs with custom attn_mask,
        # Reference: https://github.com/pytorch/pytorch/issues/112577.
        if query_states.device.type == "cuda" and attention_mask is not None:
            ...
            for image_idx in self.images_idx:
                ...
                for vi in range(image_idx[0].shape[0]):
                    if vi == 0:
                        ...
                        states_segment.append(hidden_states[i:i+1,image_idx[vi-1] + self.visual_length: image_idx[vi]])
                        position_segment.append(position_ids[i:i+1,image_idx[vi-1] + self.visual_length: image_idx[vi]])
