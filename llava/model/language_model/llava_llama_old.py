        ...
        **model_kwargs,
        ...
        while True:
            ...
            if synced_gpus:
                ...
                if this_peer_finished_flag.item() == 0.0:
                    break
            for idx, (input_ids, unfinished_sequences,this_peer_finished,delta,past_key_values,attention_mask) in \
                enumerate(zip(input_ids_list, unfinished_sequences_list,this_peer_finished_list,delta_list,past_key_values_list,attention_mask_list)):
            # prepare model inputs
                ...
                if self.cot_decoding and step == 0:
                    sorted_scores, next_tokens = torch.topk(next_tokens_scores, self.num_branch, dim=-1)
                    # split next_tokens into list
                    next_next_tokens_list = next_tokens.split(1, dim=-1)
                    next_delta_list = torch.tensor([sorted_scores[:,0] - sorted_scores[:,1]] + [0] * (self.num_branch - 1))
                    next_past_key_values_list = [past_key_values] * self.num_branch
                ...
                if step == 0 and self.cot_decoding:
                    ...
                    next_delta_list = torch.cat([torch.tensor(delta_list)[:,None],next_delta_list[:,None]],dim=1).split(1,dim=0)
                    ...
                    if isinstance(delta, int):
                        delta = torch.tensor(delta).view(1,1)
                    if next_input_ids_list is None:
                        next_input_ids_list = []
                        next_delta_list = []
                        next_unfinished_sequences_list = []
                        next_this_peer_finished_list = []
                    input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
                    delta = torch.cat([delta.to(next_delta.device), next_delta],dim=1)
                ...
                if self.cot_decoding and step == 0:
                    next_attention_mask_list = [attention_mask] * self.num_branch
                if step == 0 and self.cot_decoding:
                    for next_idx , (next_input_ids, next_unfinished_sequences, next_this_peer_finished,next_next_tokens,next_delta) in \
                        enumerate(zip(next_input_ids_list, next_unfinished_sequences_list, next_this_peer_finished_list,next_next_tokens_list,next_delta_list)):
                        if eos_token_id_tensor is not None:
                            ...
                            if next_unfinished_sequences.max() == 0:
                                next_this_peer_finished_list[next_idx] = next_this_peer_finished = True
                    ...
                    if not synced_gpus:
                        tempt_finished_input_ids = []
                        tempt_this_peer_finished_list = []
                        tempt_unfinished_sequences_list = []
                        tempt_delta_list = []
                        tempt_past_key_values_list = []
                        tempt_attention_mask_list = []
                    # remove this path from input_ids_list and unfinished_sequences_list
                        for i, finished in enumerate(next_this_peer_finished_list):
                            if not finished:
                                tempt_finished_input_ids.append(next_input_ids_list[i])
                                tempt_this_peer_finished_list.append(next_this_peer_finished_list[i])
                                tempt_unfinished_sequences_list.append(next_unfinished_sequences_list[i])
                                tempt_delta_list.append(next_delta_list[i])
                                tempt_past_key_values_list.append(next_past_key_values_list[i])
                                tempt_attention_mask_list.append(next_attention_mask_list[i])
                        next_input_ids_list = tempt_finished_input_ids
                        next_this_peer_finished_list = tempt_this_peer_finished_list
                        next_unfinished_sequences_list = tempt_unfinished_sequences_list
                        next_delta_list = tempt_delta_list
                        next_past_key_values_list = tempt_past_key_values_list
                        next_attention_mask_list = tempt_attention_mask_list
                ...
                # if eos_token was found in one sentence, set sentence to finished
                    ...
                    if this_peer_finished and not synced_gpus:
                        ...
                        finished_delta_list.append(delta)
                    else:
                        next_input_ids_list.append(input_ids)
                        next_delta_list.append(delta)
                        next_unfinished_sequences_list.append(unfinished_sequences)
                        next_this_peer_finished_list.append(this_peer_finished)
                        next_past_key_values_list.append(past_key_values)
                        next_attention_mask_list.append(attention_mask)
        ...
        if return_dict_in_generate:
            ...
            if self.cot_decoding:
                return finished_input_ids, finished_delta_list
            else:
                return input_ids
