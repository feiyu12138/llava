...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        model.model.images_idx = [(torch.tensor([35], device='cuda:0'),)]
...
def input_constructor():
    return {
        "inputs_embeds": torch.randn(1, 638, 4096).cuda().half(),
        "use_cache":True,
        "output_attentions":False,
        "attention_mask":torch.ones(1, 638).cuda(),
        "output_hidden_states":False,
        "return_dict": True,
        "position_ids":torch.arange(638).unsqueeze(0).cuda(),
    }
